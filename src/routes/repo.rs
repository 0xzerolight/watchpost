...
//!
//! One route serves four responses. A plain request renders the whole page; an
//! htmx request naming one of the three swap targets in `HX-Target` gets back
//! just that fragment. The data it loads is the same either way — the queries
...
use crate::routes::html::repo::{
    ALL_DAYS, ChartPayload, ChartSeries, PERIODS, PopularParams, RepoView, Sort, period_scope,
    popular_table, repo_body,
};
use crate::routes::html::{base, get_hx_target};
use crate::state::AppState;
use crate::types::{Event, Metric, PopularItem, PopularKind, RepoOverview};

/// The window used when `days` is absent or not one of [`PERIODS`].
...
enum Fragment {
    /// `#period-scope` — charts plus popular tables, after a period change.
    PeriodScope,
...
///
/// Query parameters:
/// * `days` ∈ {7, 30, 90, 365, -1}, default 90. `-1` means "all": the window
///   runs from the repo's first observed day to today, floored at
///   [`ALL_MIN_DAYS`]. Anything else — junk, an off-allowlist number, an empty
///   value — falls back to 90 rather than being clamped, so a mistyped URL
///   renders the default period instead of an arbitrary one.
...
pub async fn repo_page(
    ...
    Ok(match fragment(&headers) {
        ...
        Fragment::PeriodScope => period_scope(&view),
...
fn load(conn: &Connection, repo_id: i64, selected: i64) -> Result<Option<PageData>, DbError> {
    ...
    let window = resolve_window(conn, repo_id, selected)?;
    Ok(Some(PageData {
        repo,
        payload: chart_payload(conn, repo_id, window)?,
...
/// Turn the selected period into a window length in days. Every value but
/// [`ALL_DAYS`] is already one; "all" is measured from the first observation.
fn resolve_window(conn: &Connection, repo_id: i64, selected: i64) -> Result<u32, DbError> {
    if selected != ALL_DAYS {
        return Ok(u32::try_from(selected).unwrap_or(ALL_MIN_DAYS));
    }
...
/// Build the `#chart-data` payload for `days` ending today.
///
/// Every series is dense and the same length as `labels`, which the client
/// relies on twice: a category axis needs one label per point, and event
/// markers are positioned by looking their date up in `labels`.
fn chart_payload(conn: &Connection, repo_id: i64, days: u32) -> Result<ChartPayload, DbError> {
    let stars = queries::dense_series(conn, repo_id, Metric::Stars, days)?;
    let labels = stars.iter().map(|(date, _)| date.clone()).collect();
    let metric = |metric| -> Result<Vec<Option<i64>>, DbError> {
        Ok(values(queries::dense_series(conn, repo_id, metric, days)?))
    };
    Ok(ChartPayload {
        days: i64::from(days),
        labels,
        series: ChartSeries {
            ...
            downloads_total: values(queries::dense_downloads_total(conn, repo_id, days)?),
        },
    })
}
